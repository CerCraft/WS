import { analyzeUrl } from "./lib/safety";
  ...
  for (const m of mutations) {
    m.addedNodes.forEach((node) => {
      if (node instanceof HTMLElement) {
        if (node.tagName === "A") enhanceLink(node as HTMLAnchorElement);
...
function enhanceLink(a: HTMLAnchorElement) {
  if (a.dataset.wsEnhanced) return;
  a.dataset.wsEnhanced = "1";

  a.addEventListener("mouseenter", (e) => {
    if (!a.href || !a.href.startsWith("http")) return;
    showTooltip(e as MouseEvent, analyzeUrl(a.href));
  });
...
function showTooltip(e: MouseEvent, result: ReturnType<typeof analyzeUrl>) {
